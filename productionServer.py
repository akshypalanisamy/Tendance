...
db = setup.loginToDatabase("flask", "Password1#")
cur = db.cursor()
...
def index():
    return 'Hello There'#jsonify(result={"status": 200})
...
def login():
    ...
    becoded = urllib.parse.quote_from_bytes(request.get_data())
    decoded = urllib.parse.unquote(becoded)
    print(decoded)
    ...
    email = urllib.parse.unquote(email[1])
    print("Email: "+ email)
...
def createResponse():
    ...
    print(request.get_data())
    decoded = urllib.parse.quote_from_bytes(request.get_data())
...
def createEvent():
    # Decodes request into variable.
    print(request.get_data())
    bcoded = urllib.parse.quote_from_bytes(request.get_data())
    print(bcoded)
    decoded = urllib.parse.unquote(bcoded)
    print(decoded)
    decoded = decoded.split("&")
    ...
    duration = urllib.parse.unquote(duration[1])
    print("Duration: " + str(duration))

    radius = radius[1]
    eventTitle = eventTitle[1]
    eventDescription = eventDescription[1]
    eventTime = urllib.parse.unquote(eventTime[1])
    latitude = latitude[1]
    userID = userID[1]
    ...
    commitEvent(userID, eventTitle, eventDate, eventTime, duration,
                radius, eventDescription, latitude, longitude, cur, db)
    return jsonify(result={"status": 200})
...
def createAccount():
    ...
    print(response)
    decoded = urllib.parse.unquote(response)
    print(decoded)
    decoded = decoded.split("&")
    password = decoded[0].split("=")[1]
    email = decoded[2].split("=")[1]
    name = decoded[1].split("=")[1]
    print("password: " + password)
    print("name: " + name)
...
def createAttendenceRecord():
    # Get the data from the request.
    response = urllib.parse.quote_from_bytes(request.get_data())
    decoded = urllib.parse.unquote(response)
    print(decoded)
    decoded = decoded.split("&")
    event_id = decoded[0].split('=')[1]
    user_id = decoded[2].split('=')[1]
    response = decoded[1].split('=')[1]
